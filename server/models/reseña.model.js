...
const getReseña = async () => {
  const res = await pool.query('SELECT * FROM "reseña"');
  return res.rows;
};

const getReseñaByIdPedido = async (id) => {
  const res = await pool.query(`SELECT * FROM public."reseña" WHERE id_pedido = '${id}'`);
  return res.rows[0] || null;
};

const getReseñaByIdCliente = async (id) => {
  const res = await pool.query(`SELECT * FROM public."reseña" WHERE id_cliente = '${id}'`);
  return res.rows;
};
